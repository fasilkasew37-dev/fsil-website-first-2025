...
(function() {
    // ቀድሞ የተፈጠሩ ካሉ ማጥፊያ
    const old = document.getElementById('ai-root');
    if (old) old.remove();
    ...
    root.innerHTML = `
        ...
        <div id="ai-window" style="position:fixed; bottom:95px; right:20px; width:320px; height:450px; background:white; border-radius:15px; display:none; flex-direction:column; box-shadow:0 5px 25px rgba(0,0,0,0.3); z-index:2147483647; border:1px solid #ddd; font-family: sans-serif; overflow:hidden;">
            ...
            <div id="ai-messages" style="flex:1; padding:15px; overflow-y:auto; font-size:14px; display:flex; flex-direction:column; gap:10px; background:#f9f9f9; color:black !important;">
                <div style="background:#eee; padding:10px; border-radius:10px; align-self:flex-start; color:black;">ሰላም ፋሲል! አሁን መላክ ይሰራል።</div>
            </div>
    ...
    async function sendMsg() {
        ...
        uDiv.style = "align-self:flex-end; background:#0080ff; color:white; padding:10px; border-radius:10px; max-width:85%;";
        uDiv.innerText = text;
        ...
        try {
            ...
            aDiv.style = "align-self:flex-start; background:#eee; padding:10px; border-radius:10px; color:black; max-width:85%;";
            aDiv.innerHTML = "<b>🤖:</b> " + reply;
            msgs.appendChild(aDiv);
        } catch (e) {
            if(load.parentNode) msgs.removeChild(load);
            const eDiv = document.createElement('div');
            eDiv.style = "color:red; font-size:12px; align-self:center;";
