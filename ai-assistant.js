...
(function() {
    ...
    chatContainer.innerHTML = `
        ...
        <div id="ai-window" style="position:fixed; bottom:155px; right:20px; width:310px; height:420px; background:white; border-radius:15px; display:none; flex-direction:column; box-shadow:0 5px 25px rgba(0,0,0,0.3); z-index:999999; border:1px solid #ddd; font-family: sans-serif; overflow:hidden;">
            ...
            <div id="ai-messages" style="flex:1; padding:15px; overflow-y:auto; font-size:14px; display:flex; flex-direction:column; gap:10px; background:#f9f9f9; color:black !important;">
                <div style="background:#eee; padding:10px; border-radius:10px; align-self:flex-start; color:black !important;">ሰላም ፋሲል! አሁን ኮዱ ተስተካክሏል። ጥያቄህን ጻፍልኝ።</div>
            </div>
    ...
    btn.onclick = async function() {
        ...
        msgs.innerHTML += `<div style="align-self:flex-end; background:#0080ff; color:white !important; padding:10px; border-radius:10px;">${userText}</div>`;
        input.value = '';
        ...
        try {
            ...
            msgs.innerHTML += `<div style="align-self:flex-start; background:#eee; padding:10px; border-radius:10px; color:black !important;"><b>🤖:</b> ${aiReply}</div>`;
        } catch (e) {
            if(loading.parentNode) msgs.removeChild(loading);
            msgs.innerHTML += `<div style="color:red;">ስህተት ተፈጥሯል። ግንኙነትህን አረጋግጥ።</div>`;
        }
        msgs.scrollTop = msgs.scrollHeight;
    };
})();
